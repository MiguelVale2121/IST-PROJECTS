...
static pthread_mutex_t destroy_all;
static int destroy_running = 0;

int tfs_init() {
    state_init();
    if (pthread_mutex_init(&single_global_lock, 0) != 0)
        return -1;
    if (pthread_cond_init(&files_opened, NULL) != 0)
        return -1;
    if (pthread_mutex_init(&destroy_all, 0) != 0)
        return -1;
    /* create root inode */
...
int tfs_destroy_after_all_closed() {
    pthread_mutex_lock(&destroy_all);
    destroy_running = 1;
    while(open_files >0) {
        pthread_cond_wait(&files_opened, &destroy_all);
    }
    pthread_mutex_unlock(&destroy_all);
    tfs_destroy();
    return 0;
}
...
int tfs_open(char const *name, int flags) {
    ...
    if (pthread_mutex_lock(&destroy_all) != 0)
        return -1;
    open_files++;
    if (pthread_mutex_unlock(&destroy_all) != 0)
        return -1;   
...
int tfs_close(int fhandle) {
    ...
    if(open_files > 0) 
        open_files--;
    if(open_files == 0)
        pthread_cond_broadcast(&files_opened);
    if (pthread_mutex_unlock(&destroy_all) != 0)
        return -1;    
    if (pthread_mutex_unlock(&single_global_lock) != 0)
        return -1;
