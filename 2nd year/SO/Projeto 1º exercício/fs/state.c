...
pthread_mutex_t mutex_inode = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_blocks = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutex_oft = PTHREAD_MUTEX_INITIALIZER;
...
void state_init() {
    ...
    for (int i=0; i< INODE_TABLE_SIZE; i++) {
        inode_table[i].i_data_block[0] = data_block_alloc();
    }
...
int inode_create(inode_type n_type) {
    ...
    /*mutex*/
    pthread_mutex_lock(&mutex_inode);
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                if (b == -1) {
                    ...
                    pthread_mutex_unlock(&mutex_inode);
                    return -1;
                }
                ...
                pthread_mutex_init(&inode_table[inumber].mutex_inode_struct, NULL);
...
int add_to_open_file_table(int inumber, size_t offset) {
    ...
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        if (free_open_file_entries[i] == FREE) {
            ...
            pthread_mutex_init(&open_file_table[i].mutex_of, NULL);
            pthread_mutex_unlock(&mutex_oft);
            return i;
        }
    }
    pthread_mutex_unlock(&mutex_oft);
