...
import pt.tecnico.distledger.adminclient.grpc.NamingService;
import pt.ulisboa.tecnico.distledger.contract.DistLedgerCommonDefinitions.*;
import pt.ulisboa.tecnico.distledger.contract.NamingServer.ServerEntry;
...
public class CommandParser {
    ...
    void parseInput() {
        ...
        namingService.shutDownNamingService();
    }
    ...
    private void activate(String line){
        ...
        List<ServerEntry> retrievedServers = namingService.lookup("DistLedger", server);

        if (retrievedServers.isEmpty()) {
            System.out.println("Server " + server + " with service DistLedger isn't registered on the naming server.\n");
            return;
        }
        String host = retrievedServers.get(0).getHost();
        String port = retrievedServers.get(0).getPort();
