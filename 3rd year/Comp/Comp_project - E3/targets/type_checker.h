...
namespace mml
{
  ...
  class type_checker : public basic_ast_visitor
  {
    ...
    // Functions stack
    std::shared_ptr<std::vector<std::shared_ptr<mml::symbol>>> _functions;
  ...
  public:
    type_checker(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<mml::symbol> &symtab, std::shared_ptr<mml::symbol> function, basic_ast_visitor *parent) : basic_ast_visitor(compiler), _symtab(symtab), _function(function), _parent(parent)
    {
    }
  ...
  protected:
    ...
    void processBinaryExpression(cdk::binary_operation_node *const node, int lvl, bool allow_ptrs);
