...
namespace mml
{
  ...
  class xml_writer : public basic_ast_visitor
  {
    ...
    std::shared_ptr<mml::symbol> _function = nullptr;

  public:
    xml_writer(std::shared_ptr<cdk::compiler> compiler, cdk::symbol_table<mml::symbol> &symtab) : basic_ast_visitor(compiler), _symtab(symtab)
    {
    }
